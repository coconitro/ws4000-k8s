...
RUN echo "WS4000 bundle bust=${WS4000_BUST}" && \
    chmod +x /entrypoint.sh /start.sh /usr/local/bin/stream.sh /usr/local/bin/verify-gpu-stream.sh /usr/local/bin/verify-ws4000-render.sh /usr/local/bin/healthcheck.sh && \
